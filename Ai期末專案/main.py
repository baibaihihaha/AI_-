...
def get_args():
    # 創建"解析器" argparse.ArgumentParser()
...
def main():
    ...
    cap_width = args.width      # 預設 640
    cap_height = args.height    # 預設 480
    ...
    # Model load #############################################################
    # mediapipe 偵測手掌的方式 mp.solutions.hands
    mp_hands = mp.solutions.hands

    # 啟用手掌偵測
    # static_image_mode
        # = false -> 將輸入圖像視為視頻流 -> 在第一個輸入圖像偵測 -> 成功偵測後定位手的標記 -> 隨後圖像偵測到手並標記後只需跟蹤標記
        # = true  -> 每次輸入圖像都將進行手部偵測
    # max_num_hands 設定需偵測手的數量 默認為2
    # min_detection_confidence 手部檢測模型的最小置信度值被認為是成功的檢測。默認為0.5
    # min_tracking_confidence 標記跟蹤模型的最小置信度值將被視為已成功跟蹤的手部標記，否則將在下一個輸入圖像上自動調用手部檢測，默認為0.5
...
#     speech.save('mouse.mp3')
    ...
    # 建立 gTTS 物件
    # gTTS(text = 轉換的文字字串, lang = 語言, tld = 翻譯主機的所在的域)
    # 不同的 Google 域可以為給定語言生成不同的本地化“口音”
    # .save("檔名.mp3") 將語音轉存成mp3檔
    ...
    while True:
        ...
        # Process Key (ESC: end) 
        # waitKey() 表示等待與讀取使用者按下的按鍵，包含一個單位為「毫秒」的參數
        key = cv.waitKey(10)
        if key == 27:  # ESC
            break
        ...
        image.flags.writeable = True   # 將數組改為讀寫模式
        
        
        
        ####rest_result####
        # results.multi_hand_landmarks 偵測手的標記
        # Count() 計算數量
        # .most_common 計算長出現的
        ...
        # results.multi_hand_landmarks -> 透過資料排序知道每一個landmark代表的是哪一個關節點
        # 資料是已經經過標準化後的結果，包含水平(X)、垂直(Y)以及景深(Z)座標，這些座標點會根據手腕點(編號=0 landmark)進行變化。
        # results.multi_handedness 應當前的手，是左手 或 右手
        if results.multi_hand_landmarks is not None:
